...
```
api（路由层）→ services（业务层）→ core（能力组件）+ models（数据）+ utils（工具）
```

- `api/`：路由（auth / upload / ask / documents / conversations / evaluation）+ 鉴权依赖 deps
- `services/`：router（意图路由）、chat、rag、agent、ingestion、auth、conversation
- `core/`：embedding、vector_store、llm_client、reranker、tools、prompt_templates
...
- **LLM 分类**（可选开关）：规则判不出时才调用，减少 LLM 调用；
- **兜底**：无法判断 → RAG（最稳、可追溯）。
