...
V2 初版采用的租户选择是"注册时传 `tenant` 字段（默认 `DEFAULT_TENANT`）"这种**演示级**做法 ——
隔离强制点（DAO + 向量库 metadata）是对的，但**准入是假的**：任何人都能自选租户注册。

**V2.1 已补上邀请码准入**：admin 通过 `POST /admin/invites` 签发邀请码，
`REQUIRE_INVITE=true` 时注册必须持码，且**租户与角色由邀请码决定**，
请求里的 `tenant` 字段被忽略。`.env.example` 给的是 `true`（新部署即安全默认）；
代码默认 `false` 仅为不打断存量部署，**生产必须开启**。

实现时踩到并修掉的两个准入洞（都是真机/评审发现，已加回归用例）：

1. **bootstrap 例外的越权**：为了让空库能引导出第一个管理员，`BOOTSTRAP_ADMIN_USERNAME`
   免码注册。但如果例外不限租户，任何人拿这个用户名 + `{"tenant":"某个已存在租户"}`
   就能免码成为那个租户的 admin —— 直接绕过邀请码读别人的数据。
   现在收窄为 **`DEFAULT_TENANT` + 该租户尚无管理员**时才生效。
2. **新租户开通的鸡生蛋**：签码需要该租户的 admin，而新租户没有 admin。
   现在放开 **`DEFAULT_TENANT` 的管理员可以为其它租户签码**（把平台租户当作运维视角），
   非平台租户仍然严格只能管自己。

仍未做的（生产化还需要）：邀请码的**批量签发 / 邮件投递 / 与组织架构同步**；
真要对接企业身份，正确方向是 SSO/OIDC 而不是自建账号体系。
